...
class ModelBasedPolicy(object):
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        # (b) Concatenate the normalized state and action
        state_action_normalized = tf.concat([state_normalized, action_normalized], axis=1)

        # (c) Pass the concatenated, normalized state-action tensor through a neural network
        next_state_delta_normalized = utils.build_mlp(
        ...
        # (d) Unnormalize the delta state prediction...
        next_state_delta = utils.unnormalize(next_state_delta_normalized, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        delta_state_normalized = utils.normalize(delta_state, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        delta_state_pred_normalized = utils.normalize(delta_state_pred, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)

        # (c) The loss function is the mean-squared-error
        loss = tf.losses.mean_squared_error(labels=delta_state_normalized, predictions=delta_state_pred_normalized)

        # (d) Create the optimizer by minimizing the loss
